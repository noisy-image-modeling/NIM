...
class Affine(InvertibleTransform):
    ...
    def __init__(
        ...
        *,
        ...
        self.resampler = Resample(norm_coords=self.norm_coord, device=device, dtype=dtype, shift_min=shift_min)
        self.spatial_size = spatial_size
        self.mode = mode
    ...
    def __call__(
        ...
        if not do_resampling:
            ...
            if grid is None:
                ...
                if self._do_transform:
                    # set randomize=False because when _do_transform rand_affine_grid is already randomized in the beginning
                    grid = self.rand_affine_grid(grid=grid, randomize=False)
